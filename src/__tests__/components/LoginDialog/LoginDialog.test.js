...
describe('<SearchBarComponent /> wrapper test', () => {
    ...
    useAuth
        .mockReturnValue({
            ...
            loginSession: jest.fn(() => Promise.resolve({
            ...
            signInWithEmailAndPasswordHandler: jest.fn(() => Promise.resolve({
                displayName: 'Alex Barajas',
                email: 'test2@mail.com',
                darkMode: false,
                favoriteList: [],
    ...
    it('Tests form send', () => {
        ...
        expect(wrapper.exists()).toBe(true);
        const dialog = wrapper.find('#dialogLog').dive();
        dialog.find('#loginForm').simulate('submit', { preventDefault() { } });
        expect(useAuth().loginSession).toBeCalled();
        wrapper.update();
    });
    ...
    it('Tests form send with error', () => {
        ...
        expect(wrapper.exists()).toBe(true);
        const dialog = wrapper.find('#dialogLog').dive();
        useAuth().loginSession.mockRejectedValue({
    ...
    it('Tests on cancel Button', () => {
        wrapper = shallow(<LoginDialog {...props} />);
        expect(wrapper.exists()).toBe(true);
        wrapper.find('#dialogLog').dive().find('#cnlLogin').simulate('click');
    ...
    it('Tests on sign with google Button', () => {
        wrapper = shallow(<LoginDialog {...props} />);
        expect(wrapper.exists()).toBe(true);
        wrapper.find('#dialogLog').dive().find('#sbtGoogle').simulate('click');
